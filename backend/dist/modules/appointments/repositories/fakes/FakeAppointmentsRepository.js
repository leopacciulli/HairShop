...
class FakeAppointmentsRepository {
  ...
  async findAppointmentsFromUserLogged({
    user_id
  }) {
    const appointments = this.appointments.filter(appointment => appointment.user_id === user_id);
    return appointments;
  }
  ...
  async delete({
    ...
    appointments.splice(appointments.findIndex(my_appointment => my_appointment.id === appointment_id), 1);
  }
